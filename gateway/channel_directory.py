        ...
        for chat_id, friendly in id_map.items():
            ...
            if not matched:
                entries.append({
                    ...
                    "type": (
                        "group"
                        if plat_name == "feishu" and chat_id.startswith("oc_")
...
async def build_channel_directory(adapters: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    for platform in adapters:
        ...
        platforms[platform_name] = await asyncio.to_thread(
            _build_from_sessions, platform_name
        )
...
def resolve_channel_name(platform_name: str, name: str) -> Optional[str]:
    """Resolve a Feishu chat ID from a human-friendly channel name."""
    directory = load_directory()
...
def format_directory_for_display() -> str:
    ...
    channels = platforms.get("feishu", [])
    if not channels:
        return "No Feishu targets discovered yet."

    lines = ["Available Feishu targets:\n"]
    for ch in channels:
        lines.append(f"  feishu:{_channel_target_name('feishu', ch)}")

    lines.append('Use these as the "target" parameter when sending.')
    lines.append('Use bare "feishu" to send to the configured home channel.')
