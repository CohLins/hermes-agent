...
class Platform(Enum):
    """Platforms supported by this Feishu/API-focused distribution."""
...
PORT_BINDING_PLATFORM_VALUES = frozenset({"api_server", "feishu"})
...
# API Server is enabled explicitly and has no credential completeness check.
...
class GatewayConfig:
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        """Check whether a single runtime platform is sufficiently configured."""
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Only the Feishu and API Server platform blocks participate in this
            # distribution's runtime configuration.
            gateway_platforms = (
            ...
            def _merge_platform_map(source_platforms: Any) -> None:
                ...
                for plat_name, plat_block in source_platforms.items():
                    if plat_name not in _RUNTIME_PLATFORM_VALUES or not isinstance(plat_block, dict):
                        continue
                    existing = platforms_data.get(plat_name, {})
                    if not isinstance(existing, dict):
                        existing = {}
                    merged_extra = {
                        **_coerce_dict(existing.get("extra")),
                        **_coerce_dict(plat_block.get("extra")),
                    }
            ...
            def _platform_block(name: str) -> tuple[dict, bool]:
                top_level = yaml_cfg.get(name)
            ...
            feishu_block, feishu_is_toplevel = _platform_block("feishu")
            if feishu_block:
                ...
                for key in (
                    ...
                    if key in feishu_block:
                        bridged[key] = feishu_block[key]
                if "unauthorized_dm_behavior" in feishu_block:
                    bridged["unauthorized_dm_behavior"] = _normalize_unauthorized_dm_behavior(
                        feishu_block["unauthorized_dm_behavior"],
                        gw_data.get("unauthorized_dm_behavior", "pair"),
                    )
                if "notice_delivery" in feishu_block:
                    bridged["notice_delivery"] = _normalize_notice_delivery(
                        feishu_block["notice_delivery"], "public"
                    )
                if "channel_prompts" in feishu_block:
                    channel_prompts = feishu_block["channel_prompts"]
                    bridged["channel_prompts"] = (
                        {str(key): value for key, value in channel_prompts.items()}
                        if isinstance(channel_prompts, dict)
                        else channel_prompts
                    )
                if "channel_overrides" in feishu_block and isinstance(
                    feishu_block["channel_overrides"], dict
                ):
                    platforms_data.setdefault("feishu", {})["channel_overrides"] = {
                        str(channel_id): override
                        for channel_id, override in feishu_block["channel_overrides"].items()
                        if isinstance(override, dict)
                    }
                if bridged or (feishu_is_toplevel and "enabled" in feishu_block):
                    plat_data, extra = _ensure_platform_extra_dict(platforms_data, "feishu")
                    if feishu_is_toplevel and "enabled" in feishu_block:
                        plat_data["enabled"] = feishu_block["enabled"]
                ...
                try:
                    ...
                    discover_plugins()
                    feishu_entry = platform_registry.get("feishu")
                    if feishu_entry and feishu_entry.apply_yaml_config_fn:
                        seeded = feishu_entry.apply_yaml_config_fn(yaml_cfg, feishu_block)
                        if isinstance(seeded, dict) and seeded:
                            _, extra = _ensure_platform_extra_dict(platforms_data, "feishu")
                            extra.update(seeded)
                except Exception as e:
                    logger.debug("Feishu YAML config bridge failed: %s", e)
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    """Apply environment overrides for API server, Feishu, and shared settings."""
    getenv = _getenv_str
    ...
    if api_server_enabled or api_server_key:
        ...
        if port:
            try:
                api_server_config.extra["port"] = int(port)
            except ValueError:
                pass
        host = getenv("API_SERVER_HOST")
        if host:
            api_server_config.extra["host"] = host
        model_name = getenv("API_SERVER_MODEL_NAME", "")
        if model_name:
            api_server_config.extra["model_name"] = model_name

    app_id = getenv("FEISHU_APP_ID")
    app_secret = getenv("FEISHU_APP_SECRET")
    if app_id and app_secret:
        feishu_config = config.platforms.setdefault(Platform.FEISHU, PlatformConfig())
        ...
        encrypt_key = getenv("FEISHU_ENCRYPT_KEY", "")
        if encrypt_key:
            feishu_config.extra["encrypt_key"] = encrypt_key
        verification_token = getenv("FEISHU_VERIFICATION_TOKEN", "")
        if verification_token:
            feishu_config.extra["verification_token"] = verification_token
        home_channel = getenv("FEISHU_HOME_CHANNEL")
        if home_channel:
