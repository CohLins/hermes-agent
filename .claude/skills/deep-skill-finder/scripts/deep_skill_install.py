    ...
    try:
        with zipfile.ZipFile(BytesIO(data)) as zf:
            ...
            for entry in names:
                ...
                # zip-slip 防护：拒绝解压到目标目录之外的条目（含 ../ 逃逸）
                resolved = out_path.resolve()
                if resolved != skill_dir_resolved and skill_dir_resolved not in resolved.parents:
                    print(f"  ⚠ 跳过越界条目（zip-slip 防护）: {entry}", file=sys.stderr)
                    continue
                if entry.endswith("/"):
...
def main():
    ...
    if args.list:
        ...
        parser.print_help()
        print("\n示例:")
        print("  deep_skill_install.py hv-analysis --dir ~/.claude/skills")
