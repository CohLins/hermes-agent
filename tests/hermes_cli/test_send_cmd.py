...
def test_list_human_output(monkeypatch, capsys):
    ...
    fake_dir.format_directory_for_display = lambda: "Available Feishu targets:\n\n  feishu:Test Group (group)\n"
    fake_dir.load_directory = lambda: {
        "platforms": {"feishu": [{"id": "oc_test", "name": "Test Group", "type": "group"}]}
    }
...
def test_list_json(monkeypatch, capsys):
    ...
    fake_dir.load_directory = lambda: {
        "platforms": {"feishu": [{"id": "-100123", "name": "Test Group"}]}
    }
    monkeypatch.setitem(_sys.modules, "gateway.channel_directory", fake_dir)
    ...
    out = capsys.readouterr().out
    payload = json.loads(out)
    assert payload["platforms"]["feishu"][0]["name"] == "Test Group"
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    (hermes_home / "config.yaml").write_text(
        "FEISHU_HOME_CHANNEL: '5550001111'\nnested:\n  ignored: true\n"
    )
    ...
    assert os.environ.get("FEISHU_HOME_CHANNEL") == "5550001111"
