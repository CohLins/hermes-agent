...
class TestHandleVoiceCommand:
    ...
    async def test_voice_on(self, runner):
        ...
        assert runner._voice_mode["feishu:123"] == "voice_only"

    @pytest.mark.asyncio
    async def test_voice_off(self, runner):
        runner._voice_mode["feishu:123"] = "voice_only"
        event = _make_event("/voice off")
        result = await runner._handle_voice_command(event)
        assert "disabled" in result.lower()
        assert runner._voice_mode["feishu:123"] == "off"
    ...
    async def test_voice_tts(self, runner):
        ...
        assert runner._voice_mode["feishu:123"] == "all"
    ...
    async def test_persistence_saved(self, runner):
        ...
        assert data["feishu:123"] == "voice_only"

    @pytest.mark.asyncio
    async def test_persistence_loaded(self, runner):
        runner._VOICE_MODE_PATH.write_text(json.dumps({"feishu:456": "all"}))
        loaded = runner._load_voice_modes()
        assert loaded == {"feishu:456": "all"}
    ...
    def test_sync_populates_enabled_chats_from_voice_modes(self, runner):
        ...
        runner._voice_mode = {
            "feishu:off_chat": "off",
            "feishu:on_chat": "voice_only",
            "feishu:tts_chat": "all",
            "unknown_platform:999": "voice_only",  # ignored for this adapter
        }
        adapter = SimpleNamespace(
    ...
    def test_restart_restores_voice_off_state(self, runner, tmp_path):
        from gateway.config import Platform
        runner._VOICE_MODE_PATH.write_text(json.dumps({"feishu:123": "off"}))
        ...
        assert restored_runner._voice_mode["feishu:123"] == "off"
        assert adapter._auto_tts_disabled_chats == {"123"}
    ...
    async def test_per_chat_isolation(self, runner):
        ...
        assert runner._voice_mode["feishu:aaa"] == "voice_only"
        assert runner._voice_mode["feishu:bbb"] == "all"

    @pytest.mark.asyncio
    async def test_platform_isolation(self, runner):
        """Same chat_id on different platforms must not collide (#12542)."""
        first_event = _make_event("/voice on", chat_id="999")
        second_event = _make_event("/voice off", chat_id="1000")

        await runner._handle_voice_command(first_event)
        await runner._handle_voice_command(second_event)

        assert runner._voice_mode["feishu:999"] == "voice_only"
        assert runner._voice_mode["feishu:1000"] == "off"
...
class TestAutoVoiceReply:
    ...
    def _call(self, runner, voice_mode, message_type, agent_messages=None,
        ...
        chat_id = "123"
        if voice_mode != "off":
            runner._voice_mode["feishu:" + chat_id] = voice_mode
        else:
            runner._voice_mode.pop("feishu:" + chat_id, None)
...
class TestSendVoiceReply:
    ...
    async def test_auto_voice_reply_uses_thread_metadata_helper(self, runner):
        ...
        event.source.thread_id = "thread_20197"
